    ...
    \begin{minipage}{0.45\linewidth}
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                \end{pgfonlayer}
                \draw[thick, darkgreen] ($(pm-5-1.north west) + (1.5pt, -1.5pt)$) rectangle ($(pm-5-5.south east) + (-1.5pt, 1.5pt)$);
                \begin{pgfonlayer}{bg}
                    \fill[thick, blue, opacity = 0.5] ($(pm-1-5.north west) + (1.5pt, -1.5pt)$) rectangle ($(pm-5-5.south east) + (-1.5pt, 1.5pt)$);
                \end{pgfonlayer}
            \end{tikzpicture}
        \end{center}
    \end{minipage}
    ...
    \begin{minipage}{0.45\linewidth}
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                \end{pgfonlayer}
                \begin{pgfonlayer}{bg}
                    \fill[mred, fill opacity = 0.5, rounded corners = 3pt] (pm-1-3.north west) rectangle (pm-5-3.south east);
                    \fill[mred, fill opacity = 0.5, rounded corners = 3pt] (pm-2-1.north west) rectangle (pm-2-5.south east);
                \end{pgfonlayer}
            \end{tikzpicture}
        \end{center}
    \end{minipage}
